...
public class Follows extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Integer> idFollows = followDao.listIdFollows(id);
		List<User> users = new ArrayList<User>();
		for (int i = 0; i < idFollows.size(); i++) {
			users.add(accountDao.getUser(idFollows.get(i).toString()));
		...
		this.getServletContext().getRequestDispatcher("/WEB-INF/follows.jsp").forward(request, response);
	}
